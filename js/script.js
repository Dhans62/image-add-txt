...
function clearAll() {
  ...
  // Remove all li entries
  const liItems = imageSizeSettings.querySelectorAll('li');
  liItems.forEach((li) => imageSizeSettings.removeChild(li));

  // Remove all file-input-entries (but keep the file input button and sort-controls)
  const fileEntries = fileInputColumn.querySelectorAll('.file-input-entry');
  fileEntries.forEach((fe) => fileInputColumn.removeChild(fe));
